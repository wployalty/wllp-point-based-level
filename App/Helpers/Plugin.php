...
class Plugin {
	...
	public static function checkDependencies( bool $allow_exit = false ): bool {
		if ( ! self::isPHPCompatible() ) {
			...
			$message = sprintf( '%1$s requires minimum PHP version %2$s', WLLP_PLUGIN_NAME, WLLP_MINIMUM_PHP_VERSION );
		...
		if ( ! self::isWordPressCompatible() ) {
			...
			$message = sprintf( '%1$s requires minimum WordPress version %2$s', WLLP_PLUGIN_NAME,
				WLLP_MINIMUM_WP_VERSION );
		...
		if ( ! self::isWooCompatible() ) {
			...
			$message = sprintf( '%1$s requires minimum Woocommerce version %2$s', WLLP_PLUGIN_NAME,
				WLLP_MINIMUM_WC_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ! self::isLoyaltyCompatible() ) {
			// translators: 1. %s will replace plugin name, 2. %s replace WooCommerce version
			$message = sprintf( '%1$s requires minimum loyalty version %2$s', WLLP_PLUGIN_NAME,
				WLLP_MINIMUM_WLR_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : self::adminNotice( esc_html( $message ), 'error' );
